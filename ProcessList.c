...
// it from it's vector. Vector_compact *must* be called once the caller is done
// removing items.
...
void ProcessList_remove(ProcessList* this, const Process* p) {
   int idx = Vector_indexOf(this->processes, p, Process_pidEqual);
   assert(idx >= 0);
   if (idx >= 0) {
...
Process* ProcessList_getProcess(ProcessList* this, pid_t pid, bool* preExisting, Process_New constructor) {
   ...
   *preExisting = proc != NULL;
   if (proc) {
      assert(Vector_indexOf(this->processes, proc, Process_pidEqual) != -1);
      assert(proc->pid == pid);
   } else {
